...
class Encuesta_model extends CI_Model
{
    ...
    function set_encuesta($idcct, $cct, $respuestas, $idencuesta){
      $fcreacion = date("Y-m-d H:i:s");
    	$this->db->trans_start();
    	$str_query = "INSERT INTO encuesta_x_cct (id_cct, id_encuesta, f_registro) VALUES(?, ?, ?);";
		$this->db->query($str_query, array($idcct, $idencuesta, $fcreacion));
    ...
    function set_encuesta_edit($respuestas, $id_editando){
      $fmod = date("Y-m-d H:i:s");
    	$this->db->trans_start();
      $str_query1 = "UPDATE encuesta_x_cct SET f_mod = ? WHERE id_aplica = ?;";
		  $this->db->query($str_query1, array($fmod, $id_editando));
			foreach ($respuestas as $respuesta) {
    ...
    function get_reporte_excel(){
    	$str_query = "SELECT x.*, m.municipio AS nom_municipio
						FROM(
						SELECT
						en.f_registro as fec_ultimoreg, en.id_aplica, ct.cct, ct.turno, ct.nombre_ct,
